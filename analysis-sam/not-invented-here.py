...
def load_signal_data(
    data_type, config, subject="sam", recording=0, remove_references=True
):
...
def plot_bandpowers(bandpowers, electrode):
    ...
    axs[0].xaxis.set_major_formatter(
        ticker.FuncFormatter(lambda x, _: index_to_time(x, time_index))
    )
    ...
    "paths": {
        ...
        "subjects": {
            "sam": {
                ...
                "recordings": {
                    "baseline": ["20200304-144100", "20200304-144601"],
...
meditation_bandpower = get_bandpower_for_electrode(
    meditation_pd, electrode=electrode_of_interest, config=cfg
)
baseline_bandpower = get_bandpower_for_electrode(
    baseline_pd, electrode=electrode_of_interest, config=cfg
)

# <codecell>

bandpower_adelie = {
    "baseline": get_bandpower_for_electrode(
        baseline_adelie_pd, electrode=electrode_of_interest, config=cfg
    ),
    "meditation": get_bandpower_for_electrode(
        meditation_adelie_pd, electrode=electrode_of_interest, config=cfg
    ),
}
...
aggregated_power_adelie = aggregate_bandpower(
    baseline=bandpower_adelie["baseline"], signal=bandpower_adelie["meditation"]
)
aggregated_power_adelie

# <codecell>

aggregated_power_sam = aggregate_bandpower(
    baseline=baseline_bandpower, signal=meditation_bandpower
)
...
for c in [
    c for c in signals_pd.columns if c not in ["TRIGGER", "X1", "X2", "X3", "A2"]
]:
...
bandpower(signals_pd.loc[:, "T5"], 300.0, cfg["bands"]["theta"])
...
for channel in range(signals.shape[1]):
    sns.lineplot(
        data=signals[::10, channel], ax=axs[channel],
    )
